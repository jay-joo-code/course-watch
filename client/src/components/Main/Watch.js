...
const Watch = ({ user, watch, setVersion, version, loading, setLoading }) => {
  const { classNumber, section, subjectCode, title, type, _id } = watch;
  ...
  return (
    <div className="border-modern-shallow m-2 bg-white container-watch d-flex flex-row align-items-center">
      ...
      <div className="danger-area h-100" onClick={handleRemove}></div>
  </div>
  )
}
